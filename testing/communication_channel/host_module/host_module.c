...
static int start_time_usec=0;

static void checkallconsumed(int dummy){
...
retry:
	...
	struct timeval t;
	do_gettimeofday(&t);
	printk("end time_sec= %ld end time_usec=%ld  start_time_sec=%d  start_time_usec=%d \n",t.tv_sec,t.tv_usec,start_time_sec,start_time_usec);
done:
...
static int thread_fn(void *unused)
{
    ...
    while (!kthread_should_stop())
    {
		...
		int i;
		for(i=1;(i<=10000 && flag==1);i++){
			struct msg_header* header = kmalloc(sizeof(struct msg_header),GFP_KERNEL);
			header->msg_status = 1;
			header->msg_type = 100; //100 for testing
			header->msg_length = 5;
			strncpy(header->msg,"hello",5);
